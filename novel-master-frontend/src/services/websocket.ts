...
class WebSocketService {
  ...
  connect() {
    ...
    /* ════════════════════════════════════════════════════════════════════════
       PHASE 3 — New WebSocket Event Listeners
       ════════════════════════════════════════════════════════════════════════ */

    this.socket.on('learning_feedback', (data: { stored_count: number; top_corrections: Array<{ original: string; corrected: string }> }) => {
      useStore.getState().setAnalyzing(false);
    });

    this.socket.on('style_context', (data: { prompt: string; has_preferences: boolean }) => {
      useStore.getState().setAnalyzing(false);
    });
  ...
  /* ═══════════════════════════════════════════════════════════════════════════
     PHASE 3 — New WebSocket Emit Methods
     ═══════════════════════════════════════════════════════════════════════════ */

  saveAndLearn(originalAiText: string, userEditedText: string, projectId: number, userId: number): void {
    if (!this.socket) return;
    this.socket.emit('save_and_learn', {
  ...
  getStyleContext(projectId: number, userId: number): void {
    if (!this.socket) return;
    this.socket.emit('get_style_context', {
      project_id: projectId,
      user_id: userId,
    });
  }
